...
def main(argv):
	...
	for tenant in tenants:
		res= os.system("/root/cloud_deletion_specific_project/cloud_deletion_specific_project/code/config --username admin --password " + admin_pass + " --tenant " + tenant +  " --api-server 127.0.0.1 show floating-ip-pool" + " > " + "/root/cloud_deletion_specific_project/cloud_deletion_specific_project/test.csv")
		...
		with open('/root/cloud_deletion_specific_project/cloud_deletion_specific_project/test.csv', 'rb') as csvfile:
			...
			for row in rules:
				...
				comand = "/root/cloud_deletion_specific_project/cloud_deletion_specific_project/code/config --username admin --password " + admin_pass + " --tenant " + tenant +  " --api-server 127.0.0.1 delete floating-ip-pool " + "--network " + c + " "  + "'" + b + "'"	
				os.system(comand)
